...
impl ParseCallbacks for Callbacks {
	fn int_macro(&self, _name: &str, value: i64) -> Option<IntKind> {
		...
		if value >= i64::min_value() as i64 && value <= i64::max_value() as i64 && _name.starts_with(ch_layout_prefix) {
...
fn link_to_libraries(statik: bool, target_os: &str) {
	if statik {
		...
		// e.g. `ff_init_half2float_tables` (originally avutil) exported from avcodec
		// and swscale.
...
fn main() {
	// The long chain of `header` method calls for `bindgen::Builder` seems to be
	// overflowing the default stack size on Windows. The main thread appears to
	// have a hardcoded stack size which is unaffected by `RUST_MIN_STACK`. As a
	// workaround, spawn a thread here with a stack size that works expermentally,
	// and allow overriding it with `FFMPEG_SYS_BUILD_STACK_SIZE` just in case.
...
fn thread_main() {
	...
	check_features(include_paths.clone(), &[
